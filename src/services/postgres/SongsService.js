...
export default class SongsService {
  ...
  async addSong({ title, year, performer, genre, duration, albumId }) {
    ...
    const _genre = genre === "" ? null : genre;
    const _albumId = albumId === "" ? null : albumId;
    const _duration = typeof duration === "number" ? duration : null;

    const res = await this._pool.query(
      `INSERT INTO songs (id, title, year, performer, genre, duration, album_id)
     VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id`,
      [id, title, year, performer, _genre, _duration, _albumId]
    );
  ...
  async editSongById(id, payload) {
    ...
    const res = await this._pool.query(
     ...
     WHERE id=$7 RETURNING id`,
      [title, year, performer, _genre, _duration, _albumId, id]
    );
    if (!res.rowCount)
